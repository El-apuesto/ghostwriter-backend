#!/usr/bin/env python3
"""
Create a user account with 10,000 credits
"""
...
def create_user():
    """Create citieschoice@gmail.com user with 10,000 credits"""
    
    db = next(get_db())
    
    try:
        # Check if user already exists
        existing_user = db.query(User).filter(User.email == "citieschoice@gmail.com").first()
        if existing_user:
        ...
        # Create new user
        user = User(
            email="citieschoice@gmail.com",
            full_name="Phantm.ink User",
            is_active=True,
